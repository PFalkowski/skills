...
- **Item 7 of the bar is discharged with the fact-check skill, and it is mandatory here, not advisory.** Where a
  ...
  a source link in its place; otherwise, an in-repo citation or independent authoritative sources.
...
An open design fork is **not** by itself a reason to answer no to either question — decide whether the *fork* is cheap to get wrong, and admit the ticket if it is, since the worker will settle it and record the choice (Oath rule 7).

The **intended** half is the one to watch. A readiness rubric structurally cannot answer it: parked, deferred, superseded, "filed so it isn't lost", awaiting-a-decision, umbrella and discussion tickets are all perfectly well specified, so they pass every readiness check and are all wrong to build.
...
**The conductor is the wrong shape here.** `sdlc-old-fashioned` run by a conductor needs someone at its gates; a patrol has no one standing anywhere, so it dispatches the lifecycle's workflow engine instead. Pointing an unattended ranger at it guarantees the gates are either improvised past or silently dropped.
