...
- **`workhorsePath`, not `{name:}`.** Pass an absolute `scriptPath` to `workflows/sdlc-workhorse.js` in the skills repo or the installed plugin. Without it, opus tickets return blocked rather than silently degrading to a lesser process: an un-run gate is visible, a skipped one is not.
