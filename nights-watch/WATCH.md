...
A recurring `/loop` expires after 7 days, and a self-paced one is not restored on `--resume`. A watch meant to outlast either runs as a `/schedule` routine or a desktop scheduled task.
...
//         workhorsePath: '<abs path to the skills repo or plugin>/workflows/sdlc-workhorse.js',
...
//                                                  // See § Stamped output.
//         parallel: 1, maxWorkers: 3, reserve: 240000 }
...
- **`high` tickets take the `workflow()` branch, not the `agent()` one**, because [`sdlc-workhorse`](../sdlc-old-fashioned/references/workflow-mode.md) is a Workflow and a ranger has no `Workflow` tool to start one with. This is the single level of nesting `workflow()` allows — the workhorse script itself calls no `workflow()`, so the budget holds and nothing throws. Both branches share the pool, the queue, and the budget.
- **`workhorsePath`, not `{name:}`.** Pass an absolute `scriptPath` to `workflows/sdlc-workhorse.js` in the skills repo or the installed plugin. Without it, `high` tickets return blocked rather than silently degrading to a lesser process: an un-run gate is visible, a skipped one is not.
...
- **Back off the usage window, not the board.** The harness meters usage in rolling windows and cuts a session off when one is spent; a ranger cut off mid-ticket leaves a claim held and a branch half-written, and the human finds it hours later. So the reserve guard applies to the window as it does to a token target: read the harness's own signal — its usage-limit notice, and a remaining-window figure where it exposes one; never guess — and when the remainder would not cover one ticket's reserve, hold the muster unclaimed, log `hold: usage window, resets <time>`, and schedule the next wake at the reset rather than the idle tick. A workflow already in flight is left to finish; one the limit does kill is recovered by the next patrol's stale-claim reap. **This is the only backpressure the Watch applies.** The number of finished PRs awaiting merge is the human's queue, and never a reason to stop clearing ready tickets.
