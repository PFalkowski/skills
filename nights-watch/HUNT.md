...
The remedy is the override, not a commit: point `state=` (or `AGENTS_STATE`) at a durable path the operator already keeps — a synced home directory, a shared volume — so the watermark and the ledger outlive any one clone without ever entering a git remote. Committing them would buy the same sharing and is the wrong trade: on a **public** repo it publishes what rule 4 withheld, and on a private one it publishes on the day that repo is opened up.
...
**Step 1 — place the state.** Public → the state root is `~/.agents/nights-watch/<repo-slug>/` (§ Where the state root is). The in-tree default is gitignored, and on a public repo that is not enough: an ignored file still lives in the tree, one `git add -f` or a tooling change from being published. The root moves out of the repo entirely, which is the only version of "not published" that doesn't rely on anyone remembering.
