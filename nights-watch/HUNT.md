...
Committing the state in-tree would buy that sharing back, and on a **public** repo it is still forbidden: it publishes what rule 4 withheld. On a private repo it is merely a choice the operator can make by setting `state=` to a tracked path — the default does not make it for them, because a state root that is committed by default publishes on the day a private repo is opened up.
