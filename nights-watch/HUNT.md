...
> **`scriptPath`, not `{name:}`** — the same trap [WATCH.md](WATCH.md) hit with the workhorse, and the Hunt is more exposed to it: named resolution reads `.claude/workflows/` **in the repo being hunted**, which is essentially never this one. Dispatch with an absolute `scriptPath` to this repo's copy. A hunt that can't resolve the script reports the failure and stands down — it does not improvise a party from the prose above, because an un-run gate is visible and a re-invented one is not. (On Windows, if that dispatch is instead rejected as containing hidden control characters, the `.js` has CRLF line endings — pass its contents inline via `script` rather than `scriptPath`; see [WATCH.md](WATCH.md) § Dispatch.)
