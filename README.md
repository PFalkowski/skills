...
| [snapshot-terminal-sessions](snapshot-terminal-sessions/SKILL.md) | Snapshot the Windows Terminal tabs currently running Claude Code sessions and write a `wt`-based `.ps1` that recreates them later with each session *resumed*, not restarted. Since Windows Terminal exposes no live tab/pane API, it walks the OS process tree, reads each `claude.exe`'s real cwd straight out of its PEB, and emits one tab per session (`claude --continue`, or `--resume` when two sessions share a directory). |
