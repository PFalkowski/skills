...
| [clean-room](clean-room/SKILL.md) | Reimplement behaviour you have observed but may not copy — an AGPL/GPL/proprietary codebase, a competitor, an ex-employer’s prior art, an inspected binary — as two *separated* passes with a written brief as the only thing that crosses. A **study** pass may read the source and emits prose only; a **build** pass never sees the source and works from the brief; a mechanical leakage screen (`screen-brief.mjs`, which fails closed on an empty input) gates the handoff. Classifies the borrowing A/B/C/D first so Tier D stops instead of being laundered, and produces a run ledger outside the repo plus the attribution record — a defensible, auditable answer to "how do you know this was not copied?" rather than a claim. |
...
| [sdlc-workhorse](sdlc-workhorse/SKILL.md) | The autonomous counterpart to `sdlc-old-fashioned` — the same lifecycle, but as an executable [Workflow](.claude/workflows/sdlc-workhorse.js) whose gates are control flow rather than a human standing at each one. Baseline (a **red baseline throws** — no "…but that failure is unrelated" path) → spec → a fresh agent grills it → design → a fresh agent grills the plan → tracer-bullet slices → per-slice TDD where **the RED is verified by a second agent** (a test that fails on a typo is a false red and is rejected — the gate prose cannot hold, since the author is the last to notice) → fresh-agent grill of each diff with every finding refute-tested → docs shipped in the same PR → retrospective + `postmortem` discipline on the failures. Truth before all, inherited from `nights-watch`: load-bearing claims are attacked by perspective-diverse refuters and **unprovable = false**. Autonomous: reversible questions defer to the backlog with the chosen default logged; lowest-sufficient tier per phase; budget-guarded; chronicles written as agents work. It **never merges, publishes, migrates, or spends** — there is no such code path — it hands back a merge-ready report. |
