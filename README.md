...
| [code-review-grill](code-review-grill/SKILL.md) | Adversarial *grilling* code review of a branch/PR diff by a *fresh* agent that never authored the code — turns grill-me's relentless interrogation onto the diff (hunk-by-hunk: what must be true for this to be correct? what breaks it?). Runs as a single reviewer or a quorum of concern-based subagents (security / architecture / code-quality / documentation / performance / tests; the doc agent fact-checks against the web). Every finding must be verified (runnable snippet + output, in-repo proof, or authoritative deep link) before it's reported — speculation isn't a finding. Resolves the base branch → merge-base diff → ripple-effect trace → consolidates a findings table (id, per-agent severity emoji, votes, verification), then **never auto-posts** — detects the active PR and asks which findings to post as inline threads (GitHub via `gh`, Azure DevOps via `azure-devops-pr-review`). |
