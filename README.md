...
| Skill | One-liner |
|---|---|
| [auto-mode-setup](auto-mode-setup/SKILL.md) | Configure a repository tree for unattended agent runs — auto mode, permission allowlists, and the deny rules that are the actual safety boundary (the allowlist is convenience; the deny list is the boundary). Run once per machine, or when the permission posture needs revisiting. |
...
| [no-comment](no-comment/SKILL.md) | A comment is usually a failure to say it in code — fix the code first. Before writing one, try a better name, an extracted function, then a type; only when all three fail, or the case is on the skill’s allowlist, has a comment earned its place. Applies both when writing comments and when reviewing code that already carries them. |
| [omv-dev-server](omv-dev-server/SKILL.md) | Build and run a self-hosted OpenMediaVault box that is both a NAS and an agent-ready dev server — Docker services (Immich) under the OMV Compose plugin, Tailscale for phone and off-LAN access with real HTTPS and no port forwarding, SSH + Termius + tmux for shells that survive a mobile connection, and per-repo dev containers whose agents can push and open PRs with no human at a terminal. Every section is a **pair**: prose explaining why, and an idempotent `--check`-able script beside it, so the same scripts that build the box also repair it. Encodes the traps that each cost hours — the `_ssh` group gate that silently refuses logins, a home created without `/etc/skel` so `~/.profile` never sources `~/.bashrc`, single-file bind mounts that reject temp-file-and-rename, per-repo `/workspace` mounts that collapse every project's agent memory onto one key, and OMV regenerating any compose file you hand-edit. |
