...
No automatic review. Opening a PR does not trigger `code-review-grill` — it costs a fresh strongest-tier read of the repo and the diff, and most changes do not earn that. A prose diff (docs, skills, markdown, config text) never does: the human reads it in the browser faster than a reviewer can be briefed. Since this repository is prose end to end, a grill here is always on request. Elsewhere, grill on request and on executable code where being wrong is expensive — security, a public API, data, money, a migration. When one does run it is a fresh reviewer, never the session that wrote the diff, what to post is the manager's call rather than the human's, and `fix-pr` follows in hybrid mode: Sonnet fixers take the mechanical findings, the manager decides the substantive ones. The mandate is `merge=ask`: the human reviews the finished PR in the browser and merges it.
