...
`sdlc-old-fashioned` runs everything that is not a quick fix or a throwaway experiment; those skip the lifecycle, not the delivery rules below. `walk-the-dog` is a leash around a run, not a destination. The `manager` is the principal of every run: it sets `sdlc-old-fashioned`'s execution model and the model tier per phase. The autonomy dial is autonomous: the manager answers the reversible questions, and only irreversible gates stop for the human. Workers run on Sonnet; adversarial and hard-to-reverse phases run on the strongest tier available (Opus or Fable). When `/manager` is not running, the session makes these calls itself, by the rules in this file: the execution model, the tiers, which findings to post, and the `fix-pr` decisions.
