---
name: sdlc-workhorse
description: 'Runs the full software lifecycle unattended as a Workflow and hands back a merge-ready report. Use for "run the whole SDLC on this" without babysitting.'
license: MIT
metadata:
...
| **So does the signal to operate it** | Phase 10 also covers observability: production code is not done until its failure modes can be seen from outside — the new failure path logs, the job or worker reports success and failure, nothing is swallowed silently, instrumentation removed with old code is replaced. This runs unattended, so the operator who needs that signal is never the agent that shipped the change. |
...
- **Opus throughout, lowest sufficient effort — with a floor under the premise.** Every phase defaults to `opus`; workers run at `low` effort for mechanical work and `medium` otherwise. The four **premise phases** (spec, requirements grill, plan, plan review) are pinned at `opus` and **`cfg.tiers` may raise them, never lower them** — the clamp runs after the merge, so a caller passing `{grill: 'haiku'}` gets `opus` anyway and the substitution is logged. This is a floor rather than a default because a cheap premise is not a cheap run: every later phase inherits it as fact — the slicer slices against the plan, the RED test asserts the acceptance criteria, the reviewer judges the diff against the spec — so a wrong premise doesn't fail, it produces a green suite certifying the wrong behaviour. Each slice gets its own effort at slice time. Never defaults a worker to the session tier.
...
Workflow({
  ...
  args: {
    ...
    tiers: { plan: 'fable' },  // per-phase model overrides
  },
})
```
