...
Verdicts:
- **Confirmed** → the claim is true **and** it meets the bar ([code-review-grill REFERENCE, § The bar](../code-review-grill/REFERENCE.md#the-bar--what-may-be-posted-inline-or-fixed-in-this-pr)): in the diff, and of a merge-relevant kind. Two things meet the bar whatever their kind, and this carve-out wins over the kind test in Carried: a comment written by a human on a line the PR changed, and a mechanical comment (typo, import, lint, formatting, naming) on a line the PR changed. Proceed to Step 3.
- **Carried** → the claim is true, the carve-out above does not apply, and it fails the bar: a sibling or pre-existing site, or a kind that is not merge-relevant (performance, observability, test coverage, docs, architecture debt, style) with no documented gate behind it. No code, no test, no commit in this PR. File one class ticket per defect shape (reuse an existing ticket for the same shape), draft a reply naming the ticket, and mark the thread carried. A Carried security or data-loss defect is filed at blocker priority whatever the mode or policy, and named in the report.
...
6. Log the outcome per comment: `C<n> → fixed (option chosen, evidence)` / `carried <ticket>` / `refuted (evidence)` / `needs-discussion`.
...
3. **Posting follows the caller's stated policy, never a guess.** The caller may pass `reply=post|draft`, `resolve=fixed|carried|none` (`carried` resolves fixed and carried threads; `fixed` resolves fixed ones only) and `tickets=file|draft`. If the caller specified nothing, the safe default is **draft, post nothing, file nothing**: pushing the fix commit is still automatic and still subject to the still-open check in Step 4 (it is the point of the run), but replies, thread resolution and class tickets stay as drafted text in the report. A carried security or data-loss defect is filed whatever the policy; only its reply and resolution follow `reply=`/`resolve=`.
4. **Return a structured report** the caller can consume without re-reading the run: per-comment outcome (`C<n> → fixed <option> | carried <ticket> | refuted <evidence> | needs-discussion <why>`), the fix commit SHA(s), the tests added, the ripple findings handled, the class tickets filed or drafted (a security or data-loss one flagged as blocker), and the drafted (or posted) replies.
