...
Verdicts:
- **Confirmed** → the claim is true **and** it meets the bar ([code-review-grill REFERENCE, § The bar](../code-review-grill/REFERENCE.md#the-bar--what-may-be-posted-inline-or-fixed-in-this-pr)): in the diff, and of a merge-relevant kind. A human-written comment on a changed line and a mechanical comment (typo, import, lint, formatting, naming) meet the bar on their own. Proceed to Step 3.
- **Carried** → the claim is true but fails the bar: a sibling or pre-existing site, or a kind that is not merge-relevant (performance, observability, test coverage, docs, architecture debt, style) with no documented gate behind it. No code, no test, no commit in this PR. File one class ticket per defect shape (reuse an existing ticket for the same shape), draft a reply naming the ticket, and mark the thread `resolve=carried`. A Carried security or data-loss defect is filed at blocker priority and named in the report.
...
The bias ranks candidate fixes for a comment that already meets the bar; it never decides scope. Whether a comment is fixed in this PR or carried is the bar's call (Step 2), made before any candidate exists. When candidates tie on correctness, rank them by, in order:
