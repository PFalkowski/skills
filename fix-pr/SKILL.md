...
- **hybrid** (default) — mechanical comments (typo, import, lint, formatting; the list in [code-review-grill REFERENCE, § The bar](../code-review-grill/REFERENCE.md#the-bar--what-may-be-posted-inline-or-fixed-in-this-pr)) are fixed autonomously by subagents; everything substantive goes through the interactive flow. Only the substantive ones ever reach the user.
...
Verdicts:
- **Confirmed** → the claim is true **and** it meets the bar ([code-review-grill REFERENCE, § The bar](../code-review-grill/REFERENCE.md#the-bar--what-may-be-posted-inline-or-fixed-in-this-pr)): in the diff, and of a merge-relevant kind. Two comments pass the kind prong whatever their kind, scope still applying: one a human reviewer wrote (not a bot or an automated review), and a mechanical one (typo, import, lint, formatting). Proceed to Step 3.
- **Noted** → the claim is true, the comment is a bot's, it is on a changed line, and its kind is `style` and not mechanical. No code, no ticket. Draft a reply that it is noted; log it `noted`.
- **Carried** → the claim is true and it fails the bar: a sibling or pre-existing site, or a kind that is not merge-relevant (`perf`, `observability`, `tests`, `docs`, `architecture`, `style`) with no documented gate behind it. No code, no test, no commit in this PR. Draft one class ticket per defect shape (reuse an existing ticket for the same shape), draft a reply naming the ticket, and mark the thread carried; Step 4 or `tickets=` decides whether the ticket is filed. A Carried security or data-loss defect is drafted at blocker priority and named first in the report whatever the mode or policy.
...
6. Log the outcome per comment: `C<n> → fixed (option chosen, evidence)` / `carried <ticket>` / `noted` / `refuted (evidence)` / `needs-discussion`.
...
3. **Then stop and ask** — never auto-post to the review conversation (headless runs don't ask: they follow the caller's `reply=`/`resolve=`/`tickets=` policy, defaulting to draft-only — see [Headless](#headless--driven-by-another-skill)):
   ...
   - *File the Carried class tickets?* The security or data-loss one is named first.
   - *Resolve/close the threads that were fixed, and the carried and noted ones?* GitHub → resolve via GraphQL `resolveReviewThread`; Azure DevOps → set thread status `fixed`/`closed` via [AZURE-DEVOPS.md](../code-review-grill/AZURE-DEVOPS.md).
4. Post only what the user approves; post one reply first, confirm it landed, then the rest. Refuted threads are replied to but left **unresolved** unless the user says otherwise — the reviewer gets to disagree. Carried threads are resolved only once their ticket exists and the reply links it.
...
3. **Posting follows the caller's stated policy, never a guess.** The caller may pass `reply=post|draft`, `resolve=fixed|carried|none` (`carried` resolves fixed and noted threads, and carried ones only when `tickets=file` gave them a ticket; `fixed` resolves fixed ones only; a thread resolves only once its reply is posted, so `resolve=` never exceeds `reply=` and under `reply=draft` nothing resolves) and `tickets=file|draft`. If the caller specified nothing, the safe default is **draft, post nothing, file nothing**: pushing the fix commit is still automatic and still subject to the still-open check in Step 4 (it is the point of the run), but replies, thread resolution and class tickets stay as drafted text in the report. A carried security or data-loss defect heads the report as a drafted blocker ticket whatever the policy; `tickets=` decides whether it is filed.
4. **Return a structured report** the caller can consume without re-reading the run: per-comment outcome (`C<n> → fixed <option> | carried <ticket> | noted | refuted <evidence> | needs-discussion <why>`), the fix commit SHA(s), the tests added, the ripple findings handled, the class tickets filed or drafted (a security or data-loss one flagged as blocker), and the drafted (or posted) replies.

## The house bias (what "best option" means here)

The bias ranks candidate fixes for a comment that already meets the bar (Step 2); it never decides scope. When candidates tie on correctness, rank them by, in order:
