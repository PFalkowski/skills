...
public sealed partial class ProcessExternalCli : IExternalCli
{
    ...
    // The path passed here always comes from BoardHtmlReport's own write under the state root,
    // never from repository content, which is what makes Windows' UseShellExecute safe for this
    // method only: it resolves the file's default handler through the shell, but there is no
    // argument text an attacker controls for that shell to misparse.
