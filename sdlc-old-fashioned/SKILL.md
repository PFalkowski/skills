...
| 8 | **Implement → GREEN → refactor** | `tdd`, under `less-is-more` + `no-comment` | Minimal code to pass (GREEN), then refactor with tests green. Loop 7–8 per slice. Every line is written to the **[standing quality standard](#the-quality-standard--always-on-for-every-line-of-code)**. |
| 9 | **Adversarial code review** | `code-review-grill`, grading against `less-is-more` + `no-comment` | Fresh-agent grill of the diff, judged for **correctness and for the quality standard alike**. Auto-apply mechanical fixes; non-mechanical findings → **unresolved PR comments**; new work → issues. |
| 10 | **Refactor / deepen** | `improve-codebase-architecture`, `less-is-more` | Design debt and quality-standard findings the review surfaced are addressed — or consciously deferred as an issue. If it reopens behaviour, loop back to 7. |
...
| 12 | **Merge** | `merge-stack` | Green CI + review resolved + docs updated + **Phase-1 baseline re-run green** (no new lint/format/test regressions). Land — **only on explicit human go** for protected/default branches. **Board:** move the item to *Done/Closed* on merge. **Delete `prompts/sdlc-backlog.md` in the publishing commit** — it's live run scaffolding, not a deliverable; once the PR is up, the tracker/PR is the source of truth and the backlog must not linger in the merged tree. Once the PR is open and pushed, **propose removing the worktree** (Step 0.7) — confirm, never auto-delete. |
...
**The quality standard — always on for every line of code.** Two skills bind every code phase of this lifecycle. They are not optional passes to run if time allows; they are part of the gate, on the same footing as a failing test.
...
They bite at three distinct points. The implementer writes to them (Phase 8). The **fresh reviewer grades the diff against them in Phase 9** — an additive parallel path, a one-caller abstraction, or a comment a rename would have replaced is a finding exactly like a bug is, and a diff bigger than its behaviour requires does not pass the gate however green its tests are. Phase 10 is where those findings get paid down. Because each phase runs in a fresh context, **name both skills in the phase brief for Phases 8, 9 and 10** (handover protocol) — an unbriefed process does not know they are the house standard.
...
- **The quality standard is a gate, not advice.** `less-is-more` and `no-comment` bind every line of production code the lifecycle produces, and Phase 9 grades the diff against them. A diff bigger than the behaviour requires fails that gate the same way a red test does.
