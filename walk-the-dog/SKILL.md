---
name: walk-the-dog
description: 'Delegate work to subagents while the main agent vets every side-effecting action they propose, approving safe ones itself and escalating only consequential decisions. Use when the user wants a subagent to do the work but not run anything unsafe, or says "walk the dog", "keep it on a leash", or /walk-the-dog.'
license: MIT
metadata:
