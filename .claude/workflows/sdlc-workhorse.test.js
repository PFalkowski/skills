...
{
  const r = await runWorkhorse({ args: baseArgs({ tiers: { plan: 'haiku' }, startedAt: '07-20 09:30' }),
    agentFn: mkAgent() })
...
{
  const r2 = await runWorkhorse({ args: baseArgs({ tiers: { plan: 'haiku' } }), agentFn: mkAgent() })
