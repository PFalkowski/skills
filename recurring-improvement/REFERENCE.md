...
~/.agent-state/<repo-slug>/recurring-improvement/runs/<process>/YYYY-MM-DD/
  report.md             # per-run working scratch; references the previous run
  <artifacts>           # optional supporting files
```

Run reports are state, not records, so they live under the house state root
outside the tree ([agent-state.md](../docs/agent-state.md)). The INDEX row
(headline carries every still-`open` ID) and the run's PR description are what
survive; gate evidence is pasted into the PR/issue, where GitHub permalinks it.

- **INDEX.md is the ledger.** One row per run: `Date │ New │ Closed │ Regressed │ Headline`. The `Closed`/`Regressed`/`New` columns track movement vs the previous run.
