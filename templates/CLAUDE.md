...
Plan non-trivial work before editing. State assumptions per `reflect`, `fact-check` the load-bearing ones, and default the cheap ones per `whatever`. Turn a vague request into a target you can verify. When the request is ambiguous or you are confused, name what is unclear and ask instead of picking a reading.
...
- Done means proven. Never report a task complete without evidence that it works.
...
- When the user corrects you, fix the source so it does not recur: the skill via `evolve-skill`, otherwise memory.
