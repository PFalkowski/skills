...
8. **On a visibility-narrowing diff (e.g. `public`→`internal sealed`), the build compiling is NOT the all-clear — two language traps survive compilation.** (a) **Reflection/proxy reach:** a test that does `Mock<ILogger<NowInternalType>>` (Castle DynamicProxy) or otherwise constructs the type via a dynamic proxy compiles fine but throws at proxy-generation in `[SetUp]`; the fix is an extra `InternalsVisibleTo("DynamicProxyGenAssembly2")`, distinct from the `InternalsVisibleTo("<TestAssembly>")` grants. Have the reviewer grep for `Mock<...<Sealed/InternalType>>` / dynamic-proxy and confirm the proxy grant is present. (b) **Construction-site grep must include fully-qualified `new Namespace.Type(` forms, not just bare `new Type(`** — a naive bare-name grep under-counts call sites and under-grants `InternalsVisibleTo`, breaking a test build the implementer didn't run. (A `Mock<ILogger<NowInternalType>>` can compile yet throw after sealing, and fully-qualified `new Some.Namespace.Type(` call sites are easily missed by a grep that only looks for the bare `new Type(`.) This is the visibility-change sibling of rules 1–2's independence principle: the reviewer's job is to find the trap the green build hid.
