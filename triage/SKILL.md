---
name: triage
description: 'Groom an issue tracker so the backlog states what it means and autonomous agents can be pointed at it safely. Sorts tickets into independent label lanes — category, state, priority — rather than one flat list, records the repo''s own label vocabulary in a doc so every agent reads the same map, and applies one canonical bar for whether a ticket is work an unattended agent should take: is it specified well enough, AND is it actually wanted right now. The second question is the one that gets skipped, and it is why parked, deferred, superseded and filed-for-the-record tickets — all perfectly well specified — are the usual cause of an agent building something nobody asked for. Writes durable agent briefs for tickets that pass, precise missing-information notes for tickets that do not, and a persistent out-of-scope record for rejected feature requests so the same idea is not re-litigated every quarter. Never enacts a refusal it was not asked for: a judgement applied across a whole board is sometimes wrong, so declining is reported rather than labelled onto someone else''s tracker. Use when triaging incoming issues, grooming or sorting a backlog, deciding whether a ticket is ready for an AI agent or needs a human, preparing work for an unattended/AFK/overnight agent run, setting up or documenting a repo''s triage labels, writing an agent brief, closing a feature request as out of scope, or when the user says "triage", "groom the backlog", "is this ready for an agent", "what needs my attention", or invokes /triage. Supplies the readiness bar that nights-watch gates on before it dispatches.'
---
