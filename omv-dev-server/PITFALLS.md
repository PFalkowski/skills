...
| A restore fails, or every restored text value has a trailing carriage return | The dump was taken through `docker exec -t`. The pty's ONLCR rewrites every LF as CRLF, including inside the `COPY … FROM stdin` blocks whose lines are read literally on restore, so `\.` becomes `\.<CR>`. `gzip -t` passes and the size looks right | Drop the `-t`; nothing about a dump needs a tty, and it also stops stderr being folded into the archive. Existing dumps are salvageable with `gzip -dc … \| tr -d '' \| psql`. Then test a restore — this survives precisely because nobody does |
