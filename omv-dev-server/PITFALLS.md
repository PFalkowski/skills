...
| The whole box hard-freezes; the journal simply stops mid-entry with no shutdown sequence and no error | If the OS disk is NVMe, power management parked it somewhere it did not come back from. Nothing could be logged because the root filesystem was gone — which is why the log looks fine right up to the cut. **Two independent layers do this**, and fixing one leaves the other | `nvme_core.default_ps_max_latency_us=0` disables the drive's own power states (APST); `pcie_aspm.policy=performance` disables power management on the PCIe link underneath it (**not** `pcie_aspm=off` - see the next row). Set both. Confirm with `cat /proc/cmdline` after the reboot |
| The same symptom, on a box where `nvme_core.default_ps_max_latency_us=0` was already active | Not APST — and `nvme get-feature -f 0x0c` reporting `APSTE: Disabled` proves that argument worked, which is what sends you here. Suspect PCIe ASPM L1.2: it gates the link's reference clock and retrains before the next transfer, and a failed retrain drops the device off the bus while the kernel keeps running. Common on laptops, and on Lite-On/Plextor controllers | `lspci -vv -s <bdf>` for `ASPM L1 Enabled` and `L1SubCtl1: … ASPM_L1.2+`, then `pcie_aspm.policy=performance`. Read the link back afterwards — the argument being present is not the same as ASPM being off |
| `pcie_aspm=off` is on the command line and ASPM is still enabled | It is not a disable switch, and the kernel docs were corrected in 6.9 to say so: "Don't touch ASPM configuration at all. Leave any configuration done by firmware unchanged." On a box whose firmware enabled ASPM it does nothing — and it sets `aspm_disabled`, which makes the policy knob inert, so it blocks the fix it looks like | `pcie_aspm.policy=performance`, which `CONFIG_PCIEASPM_PERFORMANCE` documents as disabling L0s and L1 "even if the BIOS enabled them". Remove `pcie_aspm=off`; the two do not coexist. The tell that it did nothing is an empty `/sys/bus/pci/devices/<bdf>/link/` — the kernel never built ASPM link state |
| Is the NVMe dying? | Usually not, and the drive answers it directly. Its controller keeps a persistent 64-entry log of every command that ever failed; an empty one means the drive never saw the failure, which points at the link or the host rather than the media | `nvme smart-log /dev/nvme0` — `media_errors`, `percentage_used`, `available_spare`; and `nvme error-log /dev/nvme0` — `num_err_log_entries: 0` clears the controller. Read the PCIe side too: `/sys/bus/pci/devices/<bdf>/aer_dev_correctable` and `…_fatal`. Do all of it **before** any repair writes to the disk |
| The box still answers its console, but every disk operation returns `Input/output error`, and `Ctrl+Alt+Del` ends at `Failed to execute shutdown binary` | The root device left the bus while the kernel kept running. systemd cannot read `systemd-shutdown` to finish the reboot, and you cannot log in either, because `login` and PAM have to be read from the same disk | Press `Ctrl+Alt+Del` **more than seven times in two seconds**: systemd's `CtrlAltDelBurstAction` defaults to `reboot-force` and skips the hung shutdown. `Alt+SysRq+B` does the same from inside the kernel — but Debian's default `kernel.sysrq=438` allows `b` while denying the `t`/`w` debug dumps, and plenty of keyboards have no SysRq key. Scrollback is the only forensics left, so photograph it first |
| The systemd watchdog did not reboot a box whose disk had been gone for hours | PID 1 was alive and kept petting it. A liveness check that never touches storage cannot notice storage being gone | Do not count on it for this failure. `RuntimeWatchdogSec` catches a wedged kernel, not a wedged filesystem |
...
| A backup ran with the backup disk detached and reported success | An unmounted mount point is an ordinary directory, so the backup created it and wrote itself onto the OS disk it exists to protect | `mountpoint -q "$BACKUP_ROOT"` before anything else, and fail loudly. `[ -d … ]` passes either way — the same trap as the data disk, pointed at your only off-box copy |
| A dump in the retention directory is a few bytes | `pg_dumpall` failed, but the shell had already created the file by opening the redirect. Retention counts files, not backups, so a run of these prunes the last real one | Write to `<name>.partial`, check it with `gzip -t` and a minimum size, and rename only on success. Prune only after a dump has succeeded |
| A `pg_dumpall` archive is full of CRLF, and you are about to call it corrupt | It was taken through `docker exec -t`, whose pty rewrites every LF. **Check before you conclude:** `COPY … FROM stdin` accepts CR, LF or CRLF as its end-of-line in text format, so the carriage returns are consumed as terminators and never reach the data. A CRLF dump and a clean one restored side by side gave identical rows and the same md5 | Restore one into a throwaway container rather than reasoning about it. Still drop the `-t`: it also folds stderr into stdout, so anything `pg_dumpall` warns about lands inside the archive, and that would be real corruption |
' \| psql`. Then test a restore — this survives precisely because nobody does |
