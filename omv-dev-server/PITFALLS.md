...
| The whole box hard-freezes; the journal simply stops mid-entry with no shutdown sequence and no error | If the OS disk is NVMe, power management parked it somewhere it did not come back from. Nothing could be logged because the root filesystem was gone — which is why the log looks fine right up to the cut. **Two independent layers do this**, and fixing one leaves the other | `nvme_core.default_ps_max_latency_us=0` disables the drive's own power states (APST); `pcie_aspm=off` disables power management on the PCIe link underneath it. Set both. Confirm with `cat /proc/cmdline` after the reboot |
| The same symptom, on a box where `nvme_core.default_ps_max_latency_us=0` was already active | Not APST — and `nvme get-feature -f 0x0c` reporting `APSTE: Disabled` proves that argument worked, which is what sends you here. Suspect PCIe ASPM L1.2: it gates the link's reference clock and retrains before the next transfer, and a failed retrain drops the device off the bus while the kernel keeps running. Common on laptops, and on Lite-On/Plextor controllers | `lspci -vv -s <bdf>` for `ASPM L1 Enabled` and `L1SubCtl1: … ASPM_L1.2+`, then `pcie_aspm=off`. Read the link back afterwards — the argument being present is not the same as ASPM being off |
