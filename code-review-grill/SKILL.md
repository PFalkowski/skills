...
3. Post **only** the selected subset. Post **one** thread first, confirm it landed (numeric `id` in the response), then the rest. Each comment body includes the finding's severity, ID, description, suggested fix, and its verification artifact. A 💡 nit opens with the nit marker (REFERENCE, § The nit marker) above all of it.
