...
- **Interrogate, don't admire.** For each change ask: *what must be true for this to be correct? what input breaks it? what caller/test relied on the old behavior? what did the author assume?* That is [inversion](../invert/SKILL.md) aimed at a diff — the question that finds bugs is what would make this wrong, never what makes it right.
