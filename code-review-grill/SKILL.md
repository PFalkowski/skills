...
- **Ask "does this fix undo another fix?"** A diff that fixes several things can have one fix re-arm the defect another just removed. The shape to hunt: fix A *enriches* a value — appends a response body to an exception message, widens a payload, relaxes a parser, adds a field to a log line — while some consumer downstream still matches that value **loosely** (a substring test, a `Contains`, a truthiness check, a regex without anchors). The enrichment then satisfies the loose match by accident, and the consumer's old bug is live again through a new door. So trace every fix *forward* to the consumers of the value it changed, not only backward to the bug it closes. This is the counterpart to the sibling ripple: that one asks *where else is this shape unfixed?*, this one asks *what did fixing it here break somewhere else?*
- **Ask "what did this fix make untrue?" — and count prose and assertions as consumers.** A behaviour is consumed by code, but it is *also* consumed by every sentence that describes it and every assertion that pins it. Those two are the dangerous consumers, because neither announces the breakage: an ADR that argued a design was safe, a README, a user-facing error string, an API/tool description a model reads to decide how to call it, a doc comment deriving a constant — none of them fail a build when the behaviour underneath them changes. **A test is worse still.** A test written against the old behaviour and left green is not neutral: it is now actively pinning a false claim, and its greenness is read by the next reviewer as evidence the claim holds. So for every behavioural fix, name the sentences and the assertions that were true before it and ask which are still true: grep the docs, the ADRs and the tool/CLI descriptions for the behaviour by name, not just the symbol, and re-read the tests that changed meaning rather than only the ones that changed colour. Documentation that contradicts the shipped code is a finding of the same severity as the code defect, because it is the thing the next change will be designed against.
...
- **Executed mutation** — for any claim about what the **tests** would catch: *"this test has teeth"*, *"the suite pins that branch"*, *"the coverage here is adequate"*, *"a regression would be caught"*. **Reasoning per line about whether a test would fail is not verification, and it is wrong often enough to be worthless.** Break the code deliberately and watch the suite; a mutant that survives is the finding. Both directions need this: a reviewer claiming a gap must show a surviving mutant, and an author claiming their tests are strong must show the mutants died. The harness must apply exactly one change at a time, run the suite, restore from a **pristine copy**, and assert the restored file's **checksum** matches before the next mutant — a run that leaves a mutation behind silently poisons every result after it, and the whole working tree. Recipe and contract in [REFERENCE.md](REFERENCE.md#executed-mutation-harness).
